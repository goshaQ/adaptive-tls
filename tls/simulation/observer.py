        ...
        for trafficlight_id, skeleton in self.skeletons.items():
            ...
            while queued_junctions:
                ...
                if junction_id not in self._buffered_shapes:  # ToDo: Revise?
                    self._buffered_shapes[junction_id] = self._get_junction_shape(junction)
                ...
                for position, segment in junction['segments'].items():
                    ...
                    idx = ((slice(*np.add(np.sort(tmp[0]), b_offset[::-1])),
                            slice(*np.add(np.sort(tmp[1]), b_offset[::-1]))))
                    print(idx)
                    color[idx] = 2
                    ...
                    if neigh_junction is not None:
                        ...
                        center_offset1 = np.add(relative_offset, np.multiply(
                            (step[::-1]
                             if position in c.Position.horizontal()
                        ...
                        queued_junctions.append((neigh_junction, center_offset1))
                        # Add to the queue...
            if trafficlight_id == 'cluster_290051904_49145925':
